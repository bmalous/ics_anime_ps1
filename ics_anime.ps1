...
function Add-DownloadBytes { param([string]$Source,[long]$Bytes)if(-not$script:DownloadBytes.ContainsKey($Source)){$script:DownloadBytes[$Source]=0};$script:DownloadBytes[$Source]+=$Bytes }
...
function ConvertFrom-AnimeHtml {
    ...
    return ([Net.WebUtility]::HtmlDecode($value).Trim())
}
...
function Resolve-SeriesSynopses {
    ...
    $synopsis = @{}
    $pending  = @{}   # mediaId -> next node id to inspect
    $memo     = @{}
    ...
    foreach ($item in $Media) {
        ...
        foreach ($node in @(Get-PrequelNode $item)) {
            ...
            $memo[[int]$node.id] = $node
            $text = ConvertFrom-AnimeHtml $node.description
            if (-not (Test-IncompleteSynopsis $text)) {
                $label = if ($node.title.english) { $node.title.english } else { $node.title.romaji }
                $synopsis[[int]$item.id] = "Series synopsis from ${label}: $text"
                $resolved = $true
            } else {
        ...
        if (-not $resolved -and -not $pending.ContainsKey([int]$item.id)) {
    ...
    for ($depth = 0; $depth -lt $MaximumDepth -and $pending.Count -gt 0; $depth++) {
        ...
        foreach ($mediaId in @($pending.Keys)) {
            ...
            if (-not (Test-IncompleteSynopsis $text)) {
                $label = if ($node.title.english) { $node.title.english } else { $node.title.romaji }
                $synopsis[$mediaId] = "Series synopsis from ${label}: $text"
                continue
            }
...
function Import-AnimeDatabase {
    ...
    foreach ($key in @('OutputDirectory','PendingExport')) { if ($loaded.Contains($key)) { $database[$key] = $loaded[$key] } }
    foreach ($key in @('Selections','KnownShows','PendingUpdates','EnabledProviders')) { if ($loaded.Contains($key)) { $database[$key] = @($loaded[$key]) } }
    foreach ($key in @('Shows','Seasons','ProviderDirectory')) { if ($loaded.Contains($key) -and $loaded[$key]) { $database[$key] = $loaded[$key] } }
...
function Save-AnimeDatabase {
    ...
    $script:Db | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $script:DbPath -Encoding UTF8
    $script:DbDirty = $false
}
...
function Get-SeasonShows {
    ...
    $result = foreach ($id in $byShow.Keys) {
        ...
        $next = @($instants | Where-Object { $_.When -ge $nowUtc } | Sort-Object When | Select-Object -First 1)
        [pscustomobject]@{
            MediaId        = $id
            ...
            NextEvent      = $(if ($next.Count) { $next[0].Event } else { $null })
            NextWhen       = $(if ($next.Count) { $next[0].When } else { $null })
...
function Resolve-QuitRequest {
    ...
    Write-Host "$($script:NewLine) E  Export now$($script:NewLine) Q  Quit anyway$($script:NewLine) Esc  Back" -ForegroundColor Gray
    do { $key = (Read-MenuKey).Key } until ($key -in @('E','Q','Escape','LeftArrow','Backspace'))
...
function Show-KeyMenu {
    ...
    while ($true) {
        ...
        Write-Host ''
        Write-Host 'Up/Down Move   Enter Select   Left/Esc Back   Q Quit' -ForegroundColor DarkGray
        ...
        switch ($key) {
            ...
            default {
                if ($key -eq 'Enter' -or $key -eq 'Spacebar') { return $Items[$Selected].Value }
                if ($Back -and $key -in @('LeftArrow','Escape','Backspace')) { return $null }
            }
        }
    }
}
...
function Show-ShowDetails {
    ...
    while ($true) {
        ...
        Write-Host ''
        Write-Host 'Left/Esc/Enter Back   Q Quit' -ForegroundColor DarkGray
        $key = (Read-MenuKey).Key
        if ($key -eq 'Q') { $answer = Resolve-QuitRequest; if ($answer -ne 'BACK') { return $answer } }
        elseif ($key -in @('LeftArrow','Escape','Backspace','Enter')) { return $null }
    }
}
...
function Remove-DisabledProviderData {
    ...
    foreach($id in @($script:Db.Shows.Keys)){
        ...
        $null=$script:Db.Shows.Remove($id);$script:Db.Selections=@($script:Db.Selections|Where-Object{[string]$_-ne[string]$id});$script:Db.KnownShows=@($script:Db.KnownShows|Where-Object{[string]$_-ne[string]$id})
...
function Show-ProviderDetails {
    ...
    while($true){
        ...
        Write-Host "`nUp/Down Move   Right/Enter Show details   O Open website   Left/Esc Back   Q Quit" -ForegroundColor DarkGray
        $key=(Read-MenuKey).Key
        ...
        elseif($key-in@('RightArrow','Enter')-and$top.Count){$answer=Show-ShowDetails $top[$selected] $false;if($answer){return $answer}}
        elseif($key-eq'O'-and$Provider.Url){Start-Process $Provider.Url}
        elseif($key-in@('LeftArrow','Escape','Backspace')){return}elseif($key-eq'Q'){return Resolve-QuitRequest}
    }
}
...
function Show-ProvidersMenu {
    ...
    while($true){
        ...
        Write-Host "`nUp/Down Move   Space Toggle/Update   Right Provider details   Left/Esc Back   Q Quit" -ForegroundColor DarkGray
        $key=(Read-MenuKey).Key
        ...
        elseif($key-in@('Spacebar','Enter')-and$list.Count){
...
function Show-ToggleList {
    param([array]$Shows,[hashtable]$Excluded,[bool]$ShowExcluded,[string]$Title)
    $selected = 0
    $search = ''
    while ($true) {
        $list = @($Shows | Where-Object { $Excluded.ContainsKey($_.MediaId) -eq $ShowExcluded })
        ...
        if ($list.Count -eq 0) {
            ...
            Write-Host ''
            Write-Host '/ Filter   C Clear filter   Left/Esc Back   Q Quit' -ForegroundColor DarkGray
            $key = (Read-MenuKey).Key
            ...
            if ($key -in @('LeftArrow','Escape','Backspace')) { return $null }
            continue
        }
        ...
        for ($i = $top; $i -lt [math]::Min($top + $pageSize,$list.Count); $i++) {
            ...
            $mark   = if ($ShowExcluded) { '[ ] ' } else { '[x] ' }
            $color  = if ($i -eq $selected) { 'Yellow' } else { 'Gray' }
            Write-Host (" $cursor $mark$(Format-AnimeTitle $list[$i].Title $list[$i].Providers)") -ForegroundColor $color
        }
        Write-Host ''
        Write-Host "$($selected+1)/$($list.Count)   Up/Down Move   PgUp/PgDn   Space Toggle   Right/Enter Details   / Filter   C Clear   Left/Esc Back" -ForegroundColor DarkGray
        ...
        switch ($key) {
            ...
            'Spacebar'  {
                $show = $list[$selected]
                if ($ShowExcluded) { $null=$Excluded.Remove($show.MediaId) } else { $Excluded[$show.MediaId] = $show.Title }
            ...
            default {
                if ($key -in @('RightArrow','Enter')) {
                    $answer = Show-ShowDetails -Show $list[$selected] -Excluded $ShowExcluded
                ...
                elseif ($key -in @('LeftArrow','Escape','Backspace')) { Save-AnimeDatabase; return $null }
            }
        }
    }
}
...
function Show-ManagementMenu {
    ...
    while ($true) {
        ...
        $inCalendar = @($Shows | Where-Object { -not $Excluded.ContainsKey($_.MediaId) }).Count
        $outside    = @($Shows | Where-Object { $Excluded.ContainsKey($_.MediaId) }).Count
        $choice = Show-KeyMenu -Title "Manage $SeasonLabel shows" -Back -Items @(
            ...
            [pscustomobject]@{Label='Export calendar';Value='EXPORT'}
            [pscustomobject]@{Label='Change genre filter';Value='GENRE'}
            [pscustomobject]@{Label='Change season';Value='SEASON'}
        )
        if ($choice -in @('QUIT','EXPORT','GENRE','SEASON')) { return $choice }
        if ($null -eq $choice) { return 'GENRE' }
        if ($choice -eq 'CURRENT') {
            $answer = Show-ToggleList -Shows $Shows -Excluded $Excluded -ShowExcluded $false -Title 'Shows in the calendar - Space removes'
            if ($answer -in @('QUIT','EXPORT')) { return $answer }
        }
        elseif ($choice -eq 'EXCLUDED') {
            $answer = Show-ToggleList -Shows $Shows -Excluded $Excluded -ShowExcluded $true -Title 'Shows not added - Space adds'
            if ($answer -in @('QUIT','EXPORT')) { return $answer }
        }
    }
}
...
foreach($show in $script:Db.Shows.Values){foreach($name in @($show.Providers.Keys)){Register-Provider $name ([string]$show.Providers[$name])}}
Save-AnimeDatabase
...
function Show-PendingUpdates {
    ...
    while ($true) {
        ...
        Write-Host ''
        Write-Host 'Up/Down Move   Space Add/Remove   Right Details   D Dismiss all   Left/Esc Back' -ForegroundColor DarkGray
        $key = (Read-MenuKey).Key
        switch ($key) {
            'UpArrow'   { $selected = ($selected - 1 + $items.Count) % $items.Count }
            'DownArrow' { $selected = ($selected + 1) % $items.Count }
...
while (-not $exitRequested) {
    switch ($state) {
        ...
        'MAIN' {
            ...
            if ($script:Db.PendingExport) { $items += [pscustomobject]@{Label='Export pending changes';Value='EXPORT'} }
